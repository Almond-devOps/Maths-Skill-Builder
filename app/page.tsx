...
import SkillMapLegend from "@/components/SkillMapLegend";
    ...
    <main className="min-h-screen px-6 py-16">
      ...
      {view === "intro" && (
        <div className="mx-auto max-w-2xl">
          <div className="mb-3 overflow-hidden rounded-lg border border-paper-line bg-white/50 p-4">
            <SkillMap skillStatus={state.skillStatus} />
          </div>
          <div className="mb-8">
            <SkillMapLegend />
          </div>
          <div className="mx-auto max-w-md rounded-lg border border-paper-line bg-white/70 p-8 text-center shadow-sm">
            <p className="mb-6 font-body text-ink-soft">
              A short adaptive check walks this skill map — counting through pre-algebra — and
              stops testing a branch the moment it finds where things break down. Then it
              builds a lesson from exactly that point.
            </p>
            <button
              onClick={start}
      ...
      {view === "quiz" && currentQuestion && (
        <div className="mx-auto max-w-2xl">
          <div className="mb-8 overflow-hidden rounded-lg border border-paper-line bg-white/50 p-4">
            <SkillMap skillStatus={state.skillStatus} activeSkillId={currentQuestion.skillId} />
          </div>
          <DiagnosticQuiz
