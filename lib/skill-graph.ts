...
/**
 * Depth of each skill = longest prerequisite chain beneath it (0 for roots).
 * Used purely for laying out the visual skill map left-to-right.
 */
export function getSkillDepths(): Record<string, number> {
  const depths: Record<string, number> = {};
  function depthOf(id: string): number {
    if (depths[id] !== undefined) return depths[id];
    const skill = getSkill(id);
    const d = skill.prerequisites.length
      ? 1 + Math.max(...skill.prerequisites.map(depthOf))
      : 0;
    depths[id] = d;
    return d;
  }
  for (const skill of SKILL_GRAPH) depthOf(skill.id);
  return depths;
}
